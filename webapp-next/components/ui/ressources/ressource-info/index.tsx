...
const RessourceInfos = (props: RessourceInfosProps) => {
  ...
  const openExternalRessource = () => {
    if (ressource.kind === "file" && ressource.files) {
      window.open(ressource.files.url, "_blank", "noreferrer");
  ...
  return (
    <Box bg="#FAFCFF" borderRadius={"xl"} py="1.5rem" px="1.875rem" maxH="auto">
      {(ressource.kind === "file" ||
        ressource.kind === "link" ||
        ressource.kind === "video") && (
        <Box>
          <Heading size="sm" mb={"0.375rem"}>
            Accéder à la ressource
          </Heading>
          <Text color="#6B829B" fontSize={"sm"} my={2}>
            La ressource est ouverte à tous. Vous pouvez y accéder dès
            maintenant :
          </Text>
          <Button
            mt={2}
            ...
            size="sm"
            onClick={openExternalRessource}
          >
            {ressource.kind === "file" && (
              <>
                Télécharger le fichier <DownloadIcon ml={3} />
              </>
            )}
            {ressource.kind === "link" && (
              <>
                Accéder au site <ExternalLinkIcon ml={3} />
              </>
            )}
            {ressource.kind === "video" && (
              <>
                Accéder à la vidéo source <ExternalLinkIcon ml={3} />
              </>
            )}
          </Button>
        </Box>
